...
<template>
  <div>
      <div class="box">
        <a-table :columns="headercolumns"
                 :pagination="pagination"
                 :bordered="isbordered"
                 :row-selection="rowSelects?rowSelection:undefined"
            ...
            <span slot="icon" slot-scope="iconlist">
                <a-icon  v-for="icon in iconlist"
                   :key="icon" :type="icon" />
            </span>
            <!-- 步骤条 -->
            <span  slot="speed"  slot-scope="speend, record">
                 <span v-for="(item, index) in speedList"  :key="index">
                         <a-tag  class="speedTag" @click="step(item.status, record)"  type="primary" :style="{'cursor': item.status === speend? 'pointer': 'normal'}"  :color="item.status == speend? '#53a93f': ''" >{{item.value}}</a-tag><a-icon @click="stepnext(item.status, record)" :style="{'cursor': item.status === speend? 'pointer': 'normal', 'color':item.status == speend? '#53a93f': ''}" v-if="speedList.length - 1 !== index" class="speedIcon"   type="right-circle" />
                 </span>
            </span>
            ...
            <span slot="exre" slot-scope="record, indexitem">
              ...
              <span v-if="indexitem.button">
                  <a-button    class="button"
                      v-for="(item, index) in indexitem.button" :key="index"
                      type="primary"
