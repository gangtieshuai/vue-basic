 ...
 * @LastEditors: 郭德纲
 * @LastEditTime: 2020-11-26 11:15:26
 * @Description: 用户信息
-->
<template>
  <div>
        ...
        <a-row :gutter="24">
          ...
          <a-col :span="12">
            <a-form-item label="用户类型">
              <a-select mode="multiple" v-decorator="['usertype', { rules: [{ required: true, message: '请选择用户类型' }] },]" placeholder="请选择用户类型" @change="userTypeListSelectChange">
                <a-select-option v-for="item in userTypeList" :key="item.value" :value="item.label">{{item.value}}</a-select-option>
              </a-select>
            </a-form-item>
          </a-col>
          <a-col :span="12">
            <a-form-item label="员工性别">
              <a-select v-decorator="['gender', { rules: [{ required: true, message: '请选择员工性别' }] },]" placeholder="请选择员工性别" @change="handleSelectChange">
                <a-select-option v-for="item in sexList" :key="item.value" :value="item.label">{{item.value}}</a-select-option>
              </a-select>
            </a-form-item>
          </a-col>
          <a-col :span="12">
            <a-form-item label="出生年月">
              <a-date-picker :locale="locale" style="width: 100%;" v-decorator="['birth', { rules: [{ required: true, message: '请选择出生年月' }]}]" format="YYYY-MM-DD" />
            </a-form-item>
          </a-col>
          <a-col :span="12">
          ...
          <a-col :span="12">
            <a-form-item label="入行年月">
              <a-date-picker :locale="locale" style="width: 100%;" v-decorator="['workDate', { rules: [{ required: false, message: '请选择出生年月' }]}]" format="YYYY-MM-DD" />
            </a-form-item>
          </a-col>
          <a-col :span="12">
        ...
        <a-col :flex="1">
          <a-row type="flex">
            ...
            <a-col :flex="5">
              ...
              <div class="teacher">
                <a-row class="title" type="flex">
                  <a-col :flex="1" v-if="dataItem.empNo">员工编号: {{dataItem.empNo}}</a-col>
                  <a-col :flex="1" v-if="dataItem.orgCode">机构编号: {{dataItem.orgCode}}</a-col>
                </a-row>
              </div>
              <div class="teacher" v-if="dataItem.userType">
                用户类型:
                <a-tag style="cursor: pointer;" v-for="tag in taglist" :key="tag" color="#1890ff">{{tag}}</a-tag>
              </div>
              <div class="teacher">
                <a-row class="title" type="flex">
                  <a-col :flex="1" v-if="dataItem.phone">手机号码: {{dataItem.phone}}</a-col>
                  <a-col :flex="1" v-if="dataItem.email">电子邮件: {{dataItem.email}}</a-col>
                </a-row>
              </div>
              <div class="teacher" v-if="dataItem.idNo">身份证号码: {{dataItem.idNo}}</div>
              <div class="teacher" v-if="dataItem.des">用户描述: {{dataItem.des}}</div>
            </a-col>
          </a-row>
        </a-col>
    ...
    <a-modal v-model="uploadvisible" title="批量上传" :width="800" @ok="uploadvisible=false">
      <a-row type="flex">
        <a-col :flex="1">
          <a-button @click="down()" type="primary">下载模板</a-button>
        </a-col>
        <a-col :flex="1">
...
export default {
  ...
  data () {
    return {
      ...
      data: [],
      buttonItem: [
        { key: 'edit', value: 'edit', color: '#1890ff' },
        { key: 'del', value: 'delete', color: 'red' },
        { key: 'info', value: 'info-circle', color: '#ffae00' }
      ],
